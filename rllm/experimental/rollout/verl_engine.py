...
from rllm.experimental.rollout.rollout_engine import ModelOutput, RolloutEngine, RolloutEngineConfig
...
class VerlEngine(RolloutEngine):
    ...
    def from_config(cls, config: RolloutEngineConfig) -> "VerlEngine":
        ...
        Expected ``config.extra`` keys:
            ...
            server_manager: verl rollout server manager (Ray actor handle).
        """
        import ray
        ...
        return cls(
            ...
            server_manager=config.extra["server_manager"],
            tokenizer=tokenizer,
            processor=processor,
        )
