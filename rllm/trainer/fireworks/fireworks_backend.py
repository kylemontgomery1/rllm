...
if TYPE_CHECKING:
    ...
    from training.utils import ReconnectableClient, ResourceCleanup
...
class FireworksBackend(TinkerBackend):
    ...
    def init_rollout_engine(self, **kwargs) -> RolloutEngine:
        ...
        self.rollout_engine = FireworksEngine(
            ...
            router_replay=self._router_replay_enabled(cfg.rllm.algorithm.get("router_replay", False)),
            **rollout_extra,
        )
    ...
    def validate_config(self) -> None:
        ...
        for split in ("train", "val"):
            sp = sampling_params.get(split, {}) or {}
            if sp.get("temperature", 1.0) != 1.0 or sp.get("top_p", 1.0) != 1.0:
                logger.warning(
                    "sampling.%s.{temperature,top_p} are set away from 1.0; "
                    "this can cause issues with logprobs accuracy.",
                    split,
                )
    ...
    @staticmethod
    def _router_replay_enabled(value) -> bool:
        return value is True or value == "R3"
